...
''' THe intent of the script is to get a list of all of your static path bindings.
 ...
 you can then enter in a specific static path to see which EPGs it is bound to.
 This file is intended to be run as a script and imports the following modules:

    * get_token - Enables you to authenticate into the APIC
      and receive a token for subsequent API calls.
    * static_paths -   Includes several functions to work with static paths.'''
...
# Finds the DNs of EPGs the inputted static path is bound to.
